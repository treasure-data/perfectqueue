...
Exactly-once distributed queue built on RDBMS or SimpleDB
...
PerfectQueue uses following database schema:
  ...
  (
    id:string    -- unique key of the task
    data:blob    -- additional attributes
    timeout:int
  )

1. list: lists tasks whose timeout column is old from the backend database.
2. lock: updates timeout column of the first task
3. run: executes a command or runs a script
   * if the task takes long time, the timeout column. this is repeated until it is killed.
   * if the task takes more long time, kills the process.
4. if it succeeded, removes the row from the backend database.
5. if it failed, leave the row and expect to be retried.
...
One of --show, --push, --cancel, --configure, --exec or --run is required. The behavior of the commands is described below.
...
_Example:_
  ...
  $ perfectqueue --database sqlite://test.db --push 'task1={"attr1":"val1","attr":"val2"}'
...
_Example:_
  ...
  $ perfectqueue --database sqlite://test.db --cancel task1
  Task id=task1 is canceled.
...
_Example:_
  ...
  $ perfectqueue --configure myqueue.yaml --database mysql://root:my@localhost/mydb --run myrun.rb -- my run args
...
_Example:_
  ...
  # $ perfectqueue --database sqlite://test.db --exec ./this_file -- my run args
...
This is same as 'exec' except that it creates a instance of a class named 'Run' defined in the file. The class should has 'initialize(task)', 'run' and 'kill' method. You can get data column and id column of the task from the argument of the initialize method. It is assumed it succeeded if the method doesn't any raise errors.
...
_Example:_
  ...
  # $ perfectqueue --database sqlite://test.db --run ./this_file.rb -- my run args
